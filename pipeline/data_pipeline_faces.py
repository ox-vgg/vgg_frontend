...
IM_PATHS_FILE = 'dsetpaths.txt'
FEATURES_FILE = 'features' + os.path.sep + 'database.pkl'
...
def clear_data(DATASET_DATA_BASE_PATH):
    ...
    """
    err = ""
    dataset_im_paths = os.path.join(DATASET_DATA_BASE_PATH, IM_PATHS_FILE)
    try:
        # There can be several features files, so search
        # for them and remove them all
        filename = FEATURES_FILE.replace('.', '*.')
        for afile in glob.glob(os.path.join(DATASET_DATA_BASE_PATH, filename)):
            os.remove(afile)
        if os.path.exists(dataset_im_paths):
            os.remove(dataset_im_paths)
    except Exception as e:
        err = ""
        pass
    return err
...
def data_processing_pipeline(input_list_of_frames, input_type, index, lock, DATASET_IM_BASE_PATH, DATASET_DATA_BASE_PATH,
    ...
    """
    dataset_im_paths = os.path.join(DATASET_DATA_BASE_PATH, IM_PATHS_FILE)
    output_file = os.path.join(DATASET_DATA_BASE_PATH, FEATURES_FILE)
...
#DATASET_DATA_BASE_PATH = '/webapps/visorgen/face_search/test/data'
#DATASET_IM_BASE_PATH = '/webapps/visorgen/face_search/test/images'
#data_processing_pipeline(input_list_of_frames, 'images', 0, lock, DATASET_IM_BASE_PATH, DATASET_DATA_BASE_PATH)
...
#DATASET_DATA_BASE_PATH = '/webapps/visorgen/face_search/test/data'
#DATASET_IM_BASE_PATH = '/webapps/visorgen/face_search/test/images'
#data_processing_pipeline(inputListOfVideos, 'video', 0, lock, DATASET_IM_BASE_PATH, DATASET_DATA_BASE_PATH)
